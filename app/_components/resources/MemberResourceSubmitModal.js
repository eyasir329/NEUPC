/**
 * @file Member resource submit modal component
 * @module MemberResourceSubmitModal
 */
...
import { submitMemberResourceAction } from '@/app/_lib/actions/member-resource-submit-action';
...
const INPUT =
  'w-full rounded-xl border border-white/[0.08] bg-white/[0.04] px-3.5 py-2.5 text-[13px] text-white/90 placeholder-white/20 outline-none transition-all focus:border-white/[0.18] focus:bg-white/[0.06] focus:ring-1 focus:ring-white/[0.06]';
const LABEL = 'mb-1.5 block text-[11.5px] font-medium text-white/40';

export default function MemberResourceSubmitModal({
  categories = [],
  onClose,
}) {
  ...
  return (
    <div
      className="fixed inset-0 z-[80] flex items-end justify-center bg-black/75 px-0 backdrop-blur-sm sm:items-center sm:px-4"
      onClick={onClose}
    >
      <div
        className="relative flex max-h-[92vh] w-full flex-col overflow-hidden rounded-none border-t border-white/[0.09] bg-[#0a0b0f] shadow-2xl sm:max-w-lg sm:rounded-2xl sm:border"
        ...
        <div className="flex shrink-0 items-center gap-3 border-b border-white/[0.07] px-5 py-4">
          <div className="flex-1">
            ...
            <p className="mt-0.5 text-[11.5px] text-white/30">
              Suggest a resource for admins to review and publish
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="flex flex-col items-center gap-4 py-8 text-center">
              ...
              <div>
                ...
                <p className="mt-1.5 max-w-xs text-[12.5px] leading-relaxed text-white/40">
                  Your submission is under review. An admin will publish it once
                  approved.
                </p>
              </div>
              <button
            ...
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-start gap-2.5 rounded-xl border border-white/[0.06] bg-white/[0.02] px-3.5 py-3">
                ...
                <p className="text-[11.5px] leading-relaxed text-white/30">
                  Only URL-based resources (YouTube, links, social posts) can be
                  submitted. Files will be uploaded by admins after review.
                </p>
              </div>
              ...
              <div>
                <label className={LABEL}>
                  Resource Type <span className="text-rose-400/70">*</span>
                </label>
              ...
              <div>
                <label className={LABEL}>
                  URL <span className="text-rose-400/70">*</span>
                </label>
                <input
                  ref={firstInputRef}
                  name="embed_url"
              ...
              <div>
                <label className={LABEL}>
                  Title <span className="text-rose-400/70">*</span>
                </label>
                <input
                  name="title"
                  type="text"
              ...
              <button
                type="submit"
                disabled={pending}
                className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/[0.09] bg-white/[0.06] py-2.5 text-[13px] font-medium text-white/80 transition-all hover:bg-white/[0.10] hover:text-white disabled:pointer-events-none disabled:opacity-50"
              >
                {pending ? (
                  <>
