...
export async function execCreateEventAction(formData) {
  ...
  const payload = {
    ...
    team_size:
      formData.get('participation_type') === 'team' && formData.get('team_size')
...
export async function uploadExecEventImageAction(formData) {
  ...
  try {
    ...
    await logActivity(user.id, 'event_image_uploaded', fileId, {
      filename: file.name,
      fileId,
    });
...
export async function generateExecEventImageAction(prompt, model) {
  ...
  if (!prompt || prompt.trim().length < 3)
    return { error: 'Provide a descriptive prompt (at least 3 characters).' };
  try {
    ...
    await logActivity(user.id, 'exec_event_image_ai_generated', fileId, {
...
export async function generateExecEventTextAction(
  prompt,
  mode,
  model,
  existingContent
...
export async function execCreateBudgetEntryAction(formData) {
  ...
  if (
    !categoryType ||
    !['event', 'bootcamp', 'maintenance'].includes(categoryType)
  ) {
  ...
  if (categoryType === 'event') {
    if (!event_id)
      return { error: 'An event must be selected for Event Related budgets.' };
    eventId = event_id;
    category = 'event';
  } else if (categoryType === 'bootcamp') {
    if (!bootcamp_id)
      return {
        error: 'A bootcamp must be selected for Bootcamp Related budgets.',
      };
  ...
  await logActivity(user.id, 'exec_create_budget', 'budget_entry', data.id, {
    title,
  });
...
export async function execUpdateBudgetEntryAction(formData) {
  ...
  if (entry.approved_at)
    return { error: 'Approved budget entries cannot be modified.' };
  ...
  await logActivity(user.id, 'exec_update_budget', 'budget_entry', entryId, {
    title,
  });
...
export async function execDeleteBudgetEntryAction(formData) {
  ...
  if (fetchErr || !entry) return { error: 'Budget entry not found.' };
  if (entry.approved_at)
    return { error: 'Approved budget entries cannot be deleted.' };
  ...
  await logActivity(user.id, 'exec_delete_budget', 'budget_entry', entryId, {
    title: entry.title,
  });
