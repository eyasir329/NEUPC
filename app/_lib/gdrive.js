...
export async function uploadRecordingToDrive(buffer, filename, contentType) {
  ...
  const drive = getDriveClient();
  const folderId = await getOrCreateSubfolder(drive, rootFolderId, 'session-recordings');
  const stream = Readable.from(buffer);
...
export async function watchMeetRecordingsFolder() {
  ...
  if (appUrl.includes('localhost') || appUrl.includes('127.0.0.1')) {
    ...
    return { channelId: null, resourceId: null, folderId: null, expiration: null };
  }

  const webhookSecret = process.env.DRIVE_WEBHOOK_SECRET;
  if (!webhookSecret) throw new Error('DRIVE_WEBHOOK_SECRET env var is not set');
