...
export async function scheduleSessionAction(formData) {
  try {
    ...
    const endIso   = new Date(new Date(scheduled_at).getTime() + duration * 60_000).toISOString();
...
export async function uploadSessionRecordingAction(formData) {
  try {
    ...
    const date = (session.scheduled_at || session.session_date || new Date().toISOString()).slice(0, 10);
    const safeTopic = (session.topic || 'session').replace(/[^a-z0-9]+/gi, '-').toLowerCase().slice(0, 40);
    const filename = `${date}_${safeTopic}_${sessionId.slice(0, 8)}.${ext}`;

    const { driveUrl } = await uploadRecordingToDrive(buffer, filename, file.type);
