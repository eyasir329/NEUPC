...
import { extractDriveFileId, getYouTubeEmbedUrl } from '@/app/_lib/utils/utils';
...
const NATIVE_BROWSER_TYPES = new Set(['video/mp4', 'video/webm', 'video/ogg']);
...
export async function streamVideo(fileId, rangeHeader = null) {
  ...
  try {
    ...
    if (
      err.message &&
      err.message.includes('cannot be downloaded by the user')
    ) {
      console.error('\n======================================================');
      console.error('❌ GOOGLE DRIVE PERMISSION ERROR ❌');
      console.error(
        'The Service Account is blocked from streaming this video.'
      );
      console.error(
        'Google Drive prevents API streaming if downloads are restricted.'
      );
      console.error('');
      console.error('HOW TO FIX THIS ISSUE:');
      console.error(
        'Option A: Grant the Service Account "Editor" access to the video or folder in Google Drive.'
      );
      console.error(
        'Option B: In Drive sharing settings, click the gear icon ⚙️ and ensure'
      );
      console.error(
        '          "Viewers and commenters can see the option to download..." is CHECKED.'
      );
      console.error('======================================================\n');
      throw new Error(
        'Drive Permission Error: cannotDownloadFile (Downloads are restricted)'
      );
    }
    throw err;
  }
