    ...
    >
      ...
      {showCover && event.cover_image ? (
        ...
        <div className="z-10 hidden h-14 w-14 shrink-0 flex-col items-center justify-center rounded-xl border border-white/5 bg-gray-900/50 shadow-inner transition-all group-hover:border-violet-500/40 sm:flex">
          <span className="mb-1 text-[9px] leading-none font-bold tracking-widest text-violet-400 uppercase">{month}</span>
          <span className="text-xl leading-none font-bold text-white">{day}</span>
        </div>
      )}
              ...
              <div className="flex gap-1">
                ...
                {onDelete && (
                  <button
                    ...
                    className="flex items-center justify-center rounded-lg border border-red-500/20 p-1.5 text-red-400 hover:bg-red-500/10 transition-colors"
                  >
