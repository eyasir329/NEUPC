...
export function computeStats(variant, events) {
  return STATS_CONFIG[variant].map((s) => {
    ...
    switch (s.id) {
      ...
      case 'upcoming':
        count = events.filter(
          (e) => e._bucket === 'upcoming' || e.status === 'upcoming'
        ).length;
        break;
      case 'ongoing':
        count = events.filter((e) => e._bucket === 'ongoing').length;
        break;
      case 'completed':
        count = events.filter((e) => e._bucket === 'completed').length;
        break;
      case 'open':
        count = events.filter((e) => e._isUpcoming).length;
        break;
      case 'registered':
        count = events.filter((e) => e._userStatus === 'Registered').length;
        break;
      case 'attended':
        count = events.filter((e) => e._bucket === 'completed').length;
        break;
      case 'past':
        ...
        break;
      case 'registrations':
        count = events.reduce((sum, e) => sum + (e.registrationCount || 0), 0);
        break;
      default:
        break;
    }
