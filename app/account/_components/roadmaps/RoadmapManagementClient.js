/**
 * @file Roadmap management client component
 * @module RoadmapManagementClient
 */
...
export default function RoadmapManagementClient({ initialRoadmaps }) {
  ...
  useEffect(() => {
    setPage(1);
  }, [search, statusFilter, categoryFilter, difficultyFilter, sortKey]);
  ...
  const statusTabs = STATUS_TABS.map((t) => ({
    ...
    count:
      t.value === 'all'
        ? roadmaps.length
        : roadmaps.filter((r) => r.status === t.value).length,
  }));
        ...
        {(search ||
          ...
          <div className="flex items-center gap-2 text-xs text-gray-500">
            <span>
              ...
              of{' '}
              <span className="font-medium text-white">{roadmaps.length}</span>{' '}
              paths
            </span>
            <button
              onClick={() => {
              ...
              {paginated.map((r) => (
                <RoadmapCard
                  ...
                  onEdit={(selectedRoadmap) =>
                    setFormModal({ mode: 'edit', roadmap: selectedRoadmap })
                  }
                />
              ))}
            </div>
  ...
  return (
    <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-between">
      <span className="text-xs text-gray-500">
        ...
        of <span className="font-medium text-white">{total}</span> paths
      </span>
      <div className="flex items-center gap-1">
        <button
          onClick={() => onPage(page - 1)}
