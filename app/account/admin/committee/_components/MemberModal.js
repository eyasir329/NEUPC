...
function CommitteeAvatarCropModal({ src, onApply, onCancel, defaultAspect = '1:1' }) {
  ...
  const draw = useCallback(() => {
    ...
    ctx.strokeStyle = 'rgba(99, 102, 241, 0.6)'; // indigo border overlay
    ctx.lineWidth = 2;
    if (aspect === '1:1') {
      ctx.beginPath();
      ...
      <div className="w-full max-w-[520px] overflow-hidden rounded-2xl border border-white/8 bg-slate-900 shadow-2xl">
        ...
        <div className="mx-5 mt-8 mb-2 flex items-center gap-3">
          ...
          <div className="relative h-1.5 flex-1 rounded-full bg-slate-800">
            <div
              className="absolute top-0 left-0 h-1.5 rounded-full bg-indigo-500/70 transition-all"
        ...
        <div className="flex gap-3 border-t border-white/8 px-5 py-4">
          ...
          <button
            type="button"
            onClick={apply}
            className="flex-1 rounded-xl bg-indigo-600 px-4 py-2.5 text-xs font-semibold text-white transition hover:bg-indigo-500 active:bg-indigo-700"
          >
            Apply Crop
          </button>
        </div>
