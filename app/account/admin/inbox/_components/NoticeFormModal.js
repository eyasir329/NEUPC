...
export default function NoticeFormModal({ notice, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="border-white/10/60 max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-2xl border bg-zinc-900 shadow-2xl">
        {/* Header */}
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/10 bg-zinc-900 px-6 py-4">
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-5 p-6">
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder-zinc-500 focus:ring-2 focus:ring-sky-500 focus:outline-none"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white focus:ring-2 focus:ring-sky-500 focus:outline-none"
              >
                {NOTICE_TYPES.map((t) => {
          ...
          <div>
            ...
            <textarea
              ...
              rows={6}
              className="w-full resize-none rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder-zinc-500 focus:ring-2 focus:ring-sky-500 focus:outline-none"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full resize-none rounded-lg border border-white/10 bg-white/5 px-3 py-2 font-mono text-xs text-white placeholder-zinc-600 focus:ring-2 focus:ring-sky-500 focus:outline-none"
            />
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`relative h-6 w-11 rounded-full transition-colors duration-200 ${isPinned ? 'bg-violet-600' : 'bg-zinc-700'}`}
            >
              <span
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 rounded-xl bg-white/10 py-2.5 text-sm font-medium text-zinc-300 transition-colors hover:bg-white/15"
            >
              Cancel
            </button>
            <button
