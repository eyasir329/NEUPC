...
export default function RoadmapCard({ roadmap, onEdit }) {
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-2xl border border-white/8 bg-[#0c0e16] shadow-xl transition-all duration-300 hover:border-blue-500/30 hover:shadow-[0_8px_40px_rgba(59,130,246,0.15)] hover:-translate-y-0.5">
      {/* Thumbnail / Cover */}
      <div className="relative aspect-video w-full overflow-hidden bg-[#090b12]">
        {roadmap.thumbnail ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-blue-900/30 via-[#0a0c14] to-indigo-900/20">
            {/* Subtle grid pattern */}
        ...
        <div className="absolute top-3 left-3">
          <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold shadow-lg backdrop-blur-md ${sc.badge}`}>
            <span className={`h-1.5 w-1.5 rounded-full ${sc.dot} ${roadmap.status === 'published' ? 'animate-pulse' : ''}`} />
            {sc.label}
          </span>
        </div>
        ...
        {roadmap.estimated_duration && (
          <div className="absolute bottom-3 right-3">
            <span className="rounded-lg bg-black/70 px-2 py-0.5 text-xs font-bold text-white backdrop-blur-sm ring-1 ring-white/10 flex items-center gap-1">
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1.5">
          <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold tracking-wide uppercase ${cc.badge}`}>
            <span>{cc.icon}</span>
            {roadmap.category}
          </span>
          <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-semibold tracking-wide uppercase ${dc.badge}`}>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-1 rounded-md bg-blue-500/8 px-2 py-1 text-[11px] text-blue-300/70" title="Views">
            <Eye className="h-3 w-3" />
            <span className="tabular-nums font-medium">{(roadmap.views ?? 0).toLocaleString()}</span>
          </div>
          {roadmap.prerequisites?.length > 0 || (roadmap.prerequisites && roadmap.prerequisites.length > 0) ? (
            <div className="flex items-center gap-1 rounded-md bg-emerald-500/8 px-2 py-1 text-[11px] text-emerald-300/70" title="Prerequisites">
              <Tag className="h-3 w-3" />
              <span className="font-medium">
                {Array.isArray(roadmap.prerequisites) ? roadmap.prerequisites.length : roadmap.prerequisites.split(',').filter(Boolean).length} prereq
              </span>
            </div>
          ) : null}
        </div>
            ...
            <div className="flex items-center gap-1">
                        ...
                        return (
                          <button
                            ...
                            className={`flex w-full items-center gap-2 px-3 py-2 text-xs transition-colors hover:bg-white/5 ${
                              s === roadmap.status ? 'font-semibold text-white' : 'text-gray-400'
                            }`}
                          >
              ...
              <button
                ...
                className={`flex h-7 w-7 items-center justify-center rounded-lg transition-all ${
                  roadmap.is_featured ? 'text-amber-400 hover:bg-amber-500/15' : 'text-gray-600 hover:bg-amber-500/15 hover:text-amber-400'
                }`}
              ...
              {deleteConfirm ? (
                <div className="flex items-center gap-1">
                  ...
                  >
                    {deletePending ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Confirm'}
                  </button>
                  <button
              ...
              <button
                ...
                className="flex h-7 items-center justify-center gap-1 rounded-lg px-2 text-gray-500 transition-all hover:bg-blue-500/15 hover:text-blue-400 text-[11px] font-medium ml-1 group/edit"
                title="Edit & Stages"
              >
