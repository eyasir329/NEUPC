/**
 * @file Advisor header — greeting, role badge, current term, and at-a-glance
 *   advisory status. Mirrors the member panel header but tuned for an
 *   oversight role: shows decisions made this month and pending count
 *   instead of XP and streak.
 *
 * @module AdvisorHeader
 */

'use client';

import { motion } from 'framer-motion';
import { Shield, ClipboardCheck, Sparkles, Users } from 'lucide-react';

export default function AdvisorHeader({
  firstName = 'Advisor',
  term = '2025 – 2026',
  decisionsThisMonth = 0,
  pendingCount = 0,
  totalMembers = 0,
}) {
  return (
    <div className="relative flex flex-col items-start justify-between gap-6 overflow-hidden rounded-2xl border border-white/10 bg-zinc-900/50 p-8 backdrop-blur-xl md:flex-row md:items-center">
      <div className="pointer-events-none absolute top-0 right-0 h-64 w-64 translate-x-1/2 -translate-y-1/2 rounded-bl-full border-b border-l border-white/10 bg-white/5" />

      <div className="relative z-10 flex items-center gap-5">
        <motion.div
          initial={{ scale: 0.8, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ type: 'spring', stiffness: 300, damping: 20 }}
          className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl border border-indigo-500/20 bg-indigo-500/10 text-2xl font-bold text-indigo-400"
        >
          {firstName.substring(0, 2).toUpperCase()}
        </motion.div>
        <div>
          <p className="mb-1 text-[10px] font-bold tracking-widest text-zinc-500 uppercase">
            Welcome back
          </p>
          <h1 className="mb-2 text-3xl font-light text-zinc-100">
            {firstName}
          </h1>

          <div className="flex flex-wrap items-center gap-2">
            <motion.span
              whileHover={{ scale: 1.05 }}
              className="flex cursor-default items-center gap-1.5 rounded-2xl border border-indigo-500/30 bg-indigo-500/10 px-2.5 py-1 text-xs font-semibold tracking-tight text-indigo-300 uppercase"
            >
              <Shield className="h-3.5 w-3.5" />
              Faculty Advisor
            </motion.span>
            <motion.span
              whileHover={{ scale: 1.05 }}
              className="flex cursor-default items-center gap-1.5 rounded-2xl border border-white/10 bg-white/5 px-2.5 py-1 text-xs font-semibold tracking-tight text-zinc-400 uppercase"
            >
              <Sparkles className="h-3.5 w-3.5" />
              Term {term}
            </motion.span>
            <motion.span
              whileHover={{ scale: 1.05 }}
              className="flex cursor-default items-center gap-1.5 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 text-xs font-semibold tracking-tight text-emerald-400 uppercase"
            >
              <Users className="h-3.5 w-3.5" />
              {totalMembers} members
            </motion.span>
          </div>
        </div>
      </div>

      <div className="relative z-10 w-full rounded-2xl border border-white/10 bg-white/5 p-4 md:w-80">
        <div className="mb-3 flex items-center justify-between">
          <span className="text-[10px] font-bold tracking-widest text-zinc-500 uppercase">
            Pending decisions
          </span>
          <span className="font-mono text-xs font-medium text-zinc-100">
            {decisionsThisMonth}{' '}
            <span className="text-zinc-500">this month</span>
          </span>
        </div>
        <div className="flex items-end gap-3">
          <ClipboardCheck className="h-5 w-5 text-indigo-400" />
          <div className="flex-1">
            <div className="text-3xl font-light text-zinc-100">
              {pendingCount}
            </div>
            <div className="mt-0.5 text-[10px] font-medium tracking-widest text-zinc-500 uppercase">
              items awaiting review
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
