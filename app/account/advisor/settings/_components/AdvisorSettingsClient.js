  ...
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition-all duration-300 ${
        checked ? 'bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.4)]' : 'bg-white/10'
      }`}
    >
      <span
        ...
        <form onSubmit={handleSaveNotifications} className="space-y-4 mt-4">
          ...
          <div className="pt-3">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-600 px-5 py-2.5 text-xs font-bold text-white hover:bg-amber-700 active:scale-95 disabled:opacity-50 transition-all select-none cursor-pointer"
            >
