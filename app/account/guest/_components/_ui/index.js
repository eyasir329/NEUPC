...
export function PageHeader({ icon: Icon, title, subtitle, accent = 'blue', actions, meta }) {
  return (
    <motion.div
    ...
    >
      <div className="flex items-start gap-4">
        {Icon && (
          <div className={`rounded-xl border p-3 ${ACCENT_CHIP[accent] ?? ACCENT_CHIP.blue}`}>
...
export function SectionHeader({ icon: Icon, title, subtitle, accent = 'gray', action }) {
  return (
    <div className="mb-4 flex items-start justify-between gap-3">
      <div className="flex items-center gap-2.5">
        {Icon && <Icon className={`h-4 w-4 shrink-0 ${ACCENT_TEXT[accent] ?? ACCENT_TEXT.gray}`} />}
        <div>
...
export function IconChip({ icon: Icon, accent = 'blue', size = 'md' }) {
  const sizes = { sm: 'p-1.5 [&>svg]:h-3.5 [&>svg]:w-3.5', md: 'p-2 [&>svg]:h-5 [&>svg]:w-5', lg: 'p-2.5 [&>svg]:h-6 [&>svg]:w-6' };
  return (
    <div className={`inline-flex rounded-lg border ${ACCENT_CHIP[accent] ?? ACCENT_CHIP.blue} ${sizes[size]}`}>
      <Icon />
    </div>
  );
}

export function StatCard({ icon: Icon, label, value, sublabel, accent = 'blue', href, trend, delay = 0 }) {
  const inner = (
    <>
      <div className="flex min-h-9 items-start justify-between gap-3">
        {Icon && <IconChip icon={Icon} accent={accent} />}
        {trend && (
          <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-medium whitespace-nowrap ${
            trend.dir === 'up'
...
export function Pill({ children, tone = 'gray', className = '', icon: Icon }) {
  return (
    <span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[11px] font-medium ${PILL_TONES[tone] ?? PILL_TONES.gray} ${className}`}>
...
export function GradientBar({ value, max = 100, tone = 'blue', height = 'h-2' }) {
  ...
  return (
    <div className={`relative ${height} overflow-hidden rounded-full border border-white/[0.04] bg-white/[0.02]`}>
      <motion.div
        initial={{ width: 0 }}
        animate={{ width: `${pct}%` }}
        ...
        return (
          <button
            ...
            className={`flex flex-shrink-0 items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-all ${
              active ? 'bg-white/[0.06] text-white shadow-sm' : 'text-gray-400 hover:bg-white/[0.03] hover:text-gray-200'
            }`}
          >
            {Icon && <Icon className="h-4 w-4" />}
            {t.label}
            {typeof t.count === 'number' && (
              <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-semibold ${active ? 'bg-white/10 text-white' : 'bg-white/5 text-gray-500'}`}>
                {t.count}
              </span>
            )}
          </button>
...
export function EmptyState({ icon: Icon, title, description, action, accent = 'gray' }) {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      {Icon && (
        <div className={`mb-3 rounded-2xl border p-3 ${ACCENT_CHIP[accent] ?? ACCENT_CHIP.gray}`}>
...
export function Avatar({ name = '?', size = 'md', src }) {
  ...
  if (src) {
    ...
    return <img src={src} alt={name} className={`${sizes[size]} rounded-full border border-white/10 object-cover`} />;
  }
  const initials = name.split(' ').filter(Boolean).slice(0, 2).map((p) => p[0]?.toUpperCase()).join('');
  const tone = AVATAR_TONES[hashName(name) % AVATAR_TONES.length];
  return (
    <div className={`${sizes[size]} flex shrink-0 items-center justify-center rounded-full border border-white/10 bg-gradient-to-br font-semibold ${tone}`}>
      {initials || '?'}
    </div>
  );
}
  ...
  return (
    <>
      {Array.isArray(children)
        ? children.map((c, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: i * delay }}>
              {c}
            </motion.div>
          ))
        : children}
