/**
 * @file Guest settings client component
  ...
  return (
    <button
      ...
      className={`relative h-6 w-11 shrink-0 rounded-full border transition-all duration-300 ${
        on
          ? 'border-indigo-500/30 bg-indigo-500/20 shadow-[0_0_8px_rgba(99,102,241,0.25)]'
    ...
    >
      <motion.span
        ...
        className={`absolute top-0.5 h-4.5 w-4.5 rounded-full border transition-colors duration-200 ${
          on
            ? 'right-0.5 border-indigo-400/50 bg-indigo-400'
            : 'left-0.5 border-white/20 bg-zinc-600'
        }`}
    ...
    <div className="flex items-center justify-between gap-4 py-4.5 first:pt-2 last:pb-2">
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2.5">
          <p
            className={`text-[13px] font-bold ${disabled ? 'text-zinc-500' : 'text-zinc-200'}`}
          >
            {label}
          </p>
          {memberOnly && (
        ...
        {desc && (
          <p className="mt-1 text-xs leading-normal font-semibold text-zinc-500">
            {desc}
          </p>
        )}
      </div>
...
function EditPersonalForm({ user, onCancel, onSaved }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-5 grid gap-4 sm:grid-cols-2">
      ...
      {error && (
        <div className="flex items-center gap-2 rounded-xl border border-rose-500/20 bg-rose-500/5 px-3 py-2.5 text-xs font-bold text-rose-300 shadow-md sm:col-span-2">
      ...
      <AnimatePresence mode="popLayout" initial={false}>
        {tab === 'account' && (
          <motion.div
            ...
            className="space-y-6"
          >
            <GlassCard className="border border-white/10 bg-zinc-900/50 p-5 shadow-xl backdrop-blur-xl">
              <SectionHeader
                icon={User}
            ...
            <GlassCard className="relative overflow-hidden border border-indigo-500/20 bg-linear-to-br from-zinc-950 via-zinc-900/60 to-indigo-950/20 p-5 shadow-xl">
              <div
                aria-hidden
              ...
              <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center">
                <div className="flex h-10 w-10 shrink-0 animate-pulse items-center justify-center rounded-xl border border-indigo-500/20 bg-indigo-500/10 text-indigo-400 shadow-inner">
            ...
            <GlassCard className="border border-white/10 bg-zinc-900/50 p-5 shadow-xl backdrop-blur-xl">
              ...
              <div className="mt-2 divide-y divide-white/5">
                <ToggleRow
                  ...
                  desc="Receive general newsletter updates via verified email logs"
                  checked={prefs.emailNotices}
                  onChange={(v) => togglePref('emailNotices', v)}
                />
                <ToggleRow
                  label="Browser Triggers"
                  desc="Pop system push notices when new events are published"
                  checked={prefs.browserNotices}
                  onChange={(v) => togglePref('browserNotices', v)}
                />
                <ToggleRow
                  label="Event Check-in Reminders"
                  desc="Receive countdown triggers 24h prior to registered bootcamp workshops"
                  checked={prefs.eventReminders}
                  onChange={(v) => togglePref('eventReminders', v)}
                />
                <ToggleRow
                  label="Weekly Digests"
                  desc="Aggregate summaries of all club activities and programming contests"
                  checked={prefs.weeklyDigest}
                  onChange={(v) => togglePref('weeklyDigest', v)}
                />
                <ToggleRow
                  label="AI Event Recommendations"
                  desc="Context-based event suggestions curated specifically to your department skills"
                  checked={false}
                  onChange={() => {}}
                  disabled
                  memberOnly
        ...
        {tab === 'security' && (
          ...
          >
            <GlassCard className="border border-white/10 bg-zinc-900/50 p-5 shadow-xl backdrop-blur-xl">
              ...
              <div className="flex items-center gap-3 rounded-xl border border-white/5 bg-zinc-950/45 px-4 py-3.5 text-xs leading-relaxed font-semibold text-zinc-500 shadow-inner">
                <Shield className="h-5 w-5 shrink-0 text-emerald-400" />
                This guest account is securely authenticated using Google OAuth.
                Profile fields are synced directly from Google accounts.
              </div>
              ...
              {user.suspension_expires_at &&
                new Date(user.suspension_expires_at) > new Date() && (
                  <div className="mt-4 flex items-start gap-2.5 rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 shadow-md">
                    <Clock className="mt-0.5 h-4.5 w-4.5 shrink-0 animate-pulse text-amber-400" />
                    <div className="text-xs leading-normal font-bold">
                      <p className="text-amber-300">
                        Account status under suspension
                      </p>
                      <p className="mt-0.5 text-amber-400/60">
                        Expires: {formatDate(user.suspension_expires_at)}
                      </p>
              ...
              <div className="mt-2 flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-0.5 text-[11px] leading-normal font-semibold text-zinc-500">
                    Terminate your current dashboard browser credentials.
                  </p>
                </div>
