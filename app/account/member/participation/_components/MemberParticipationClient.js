...
import {
  CalendarDays, Trophy, Award, MessageSquare, ExternalLink,
  CheckCircle2, Clock, XCircle, TrendingUp, Medal,
  BarChart3, Download, Eye, ChevronRight, Users, Check,
  MapPin, Activity, Calendar, Ticket, CalendarCheck,
  Star, Lock, Filter, Shield, FileText, X
} from 'lucide-react';
import {
  PageHeader, StatCard, GlassCard, SectionHeader, GradientBar,
  EmptyState, Pill, TabBar, ActionButton
...
const CERT_TYPE = {
  participation: {
    label: 'Participation', tone: 'blue',
    bannerGlow: 'rgba(96,165,250,0.18)', sealGradient: 'linear-gradient(135deg,#60a5fa,#2563eb)'
  },
  completion: {
    label: 'Completion', tone: 'violet',
    bannerGlow: 'rgba(124,131,255,0.18)', sealGradient: 'linear-gradient(135deg,#7c83ff,#5b62cc)'
  },
  achievement: {
    label: 'Achievement', tone: 'amber',
    bannerGlow: 'rgba(251,191,36,0.18)', sealGradient: 'linear-gradient(135deg,#fbbf24,#d97706)'
  },
  appreciation: {
    label: 'Appreciation', tone: 'fuchsia',
    bannerGlow: 'rgba(217,70,239,0.18)', sealGradient: 'linear-gradient(135deg,#e879f9,#c026d3)'
  },
};
...
function TimelineTab({ registrations, contestParticipations, myThreads }) {
  const items = useMemo(() => {
    const list = [
      ...registrations.map((r) => ({
        id: `e-${r.id}`, type: 'event', kind: 'Registered',
        title: r.events?.title ?? 'Event', date: r.registered_at,
      })),
      ...contestParticipations.map((c) => ({
        id: `c-${c.id}`, type: 'contest',
        kind: c.rank != null ? `Rank #${c.rank}` : 'Participated',
        title: c.contests?.title ?? 'Contest', date: c.registered_at,
      })),
      ...myThreads.map((t) => ({
        id: `d-${t.id}`, type: 'discussion', kind: 'Thread authored',
        title: t.title, date: t.created_at,
      })),
    ];
  ...
  if (!items.length)
    return (
      <EmptyState
        icon={TrendingUp} title="No activity yet"
    ...
    <div className="flex flex-col">
      {items.map((item, i) => {
        ...
        return (
          ...
          >
            <div className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border bg-${cfg.tone}-500/10 border-${cfg.tone}-500/20 text-${cfg.tone}-400`}>
...
function EventsTab({ registrations }) {
  ...
  if (!registrations.length)
    return (
      <EmptyState icon={CalendarDays} title="No event registrations" description="Register for upcoming events to track your participation." />
    );

  const filterOpts = ['all', 'attended', 'confirmed', 'registered', 'cancelled'];
  const tabs = filterOpts.map((s) => ({
    value: s, label: s === 'all' ? 'All' : regStatus(s).label,
        ...
        {filtered.map((reg, i) => {
          ...
          return (
            <motion.div
              ...
              className="flex gap-4 border-b border-white/[0.06] py-3 last:border-b-0 items-center"
            >
              <div className="flex w-14 shrink-0 flex-col items-center rounded-xl border border-white/[0.06] bg-white/[0.02] py-1.5 text-center">
                <span className="text-[9.5px] font-semibold uppercase tracking-[0.1em] text-violet-400">{mo}</span>
              ...
              <div className="flex shrink-0 flex-col items-end gap-2">
                ...
                {reg.events?.slug && (
                  <a href={`/events/${reg.events.slug}`} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-gray-300">
...
function AchievementsTab({ memberAchievements = [] }) {
  ...
  const earned = useMemo(() => memberAchievements.map((item) => {
    const a = item.achievements ?? item;
    return {
      id: item.id ?? a.id, title: a.title, description: a.description || a.contest_name,
      date: a.achievement_date, year: a.year, result: a.result, category: a.category,
      isTeam: a.is_team, teamName: a.team_name,
    };
  }), [memberAchievements]);

  const categories = useMemo(() => [...new Set(earned.map((a) => a.category).filter(Boolean))], [earned]);
  ...
  const progressItems = useMemo(() => {
    const counts = categories.map((category) => ({
      category, count: earned.filter((a) => a.category === category).length,
    }));
    return counts.slice(0, 4).map((item, index) => {
      const target = Math.max(5, item.count + 3);
      return {
        id: item.category, title: `${item.category} Master`, subtitle: `${target - item.count} more to unlock`,
        value: item.count, target, tone: PROGRESS_TONES[index % PROGRESS_TONES.length],
      };
    });
  }, [categories, earned]);

  if (!earned.length) return <EmptyState icon={Trophy} title="No achievements yet" description="Contest results added by admins will appear here." />;
  ...
  const tabs = [
    ...
    ...categories.map((c) => ({ value: c, label: c, count: earned.filter((a) => a.category === c).length })),
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        {filtered.map((ach, i) => {
          ...
          return (
            <motion.div
              key={ach.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: i * 0.03 }}
            ...
            >
              <div className={`flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br ${t.gradient} shadow-lg ring-1 ring-white/10 transition-transform group-hover:scale-105`}>
                <Icon className="h-5 w-5 text-white drop-shadow" />
              </div>
              <p className="line-clamp-2 text-[13px] font-semibold text-gray-200">{ach.title}</p>
              {ach.result && <Pill tone={t.tone === 'gray' ? 'gray' : t.tone}>{ach.result}</Pill>}
              <p className="line-clamp-2 text-[10px] text-gray-500">{ach.description || `${t.label} result`}</p>
              <div className="mt-auto flex flex-wrap items-center justify-center gap-1 pt-1">
                <span className="text-[10px] text-gray-600">{ach.date ? fmtDate(ach.date) : ach.year}</span>
              </div>
            </motion.div>
          );
        })}
      ...
      {progressItems.length > 0 && (
        <>
          <div className="mt-8">
            <SectionHeader icon={Star} title="In Progress" subtitle="Keep pushing to unlock the next badge" accent="violet" />
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {progressItems.map((item, i) => (
                <motion.div key={item.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
                  <div className="mb-3 flex items-start justify-between gap-3">
                    ...
                    <Pill tone={item.tone}>{item.value} / {item.target}</Pill>
                  </div>
      ...
      <div className="mt-8">
        <SectionHeader icon={Lock} title="Locked Achievements" subtitle="Goals waiting to be unlocked" accent="gray" />
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
          {lockedItems.map((item, i) => (
            <motion.div key={item.id} initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.2, delay: i * 0.025 }} className="flex flex-col items-center gap-1.5 rounded-lg border border-white/[0.06] bg-white/[0.015] px-3 py-3.5 text-center opacity-70 transition-all hover:opacity-100">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm" onClick={onClose}>
      <motion.div
        ...
        className="relative w-full max-w-2xl overflow-hidden rounded-2xl border border-white/[0.08] bg-gray-950 shadow-2xl" onClick={e => e.stopPropagation()}
      >
        <button onClick={onClose} className="absolute right-4 top-4 z-10 rounded-lg p-2 text-gray-400 hover:bg-white/[0.08] hover:text-gray-200"><X size={18} /></button>
        <div className="flex items-center justify-between gap-4 border-b border-white/[0.06] px-6 py-6 sm:px-8 sm:py-8" style={{ background: `radial-gradient(circle at 80% 30%, ${meta.bannerGlow}, transparent 60%), rgba(255,255,255,0.02)` }}>
          <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full text-white shadow-lg" style={{ background: meta.sealGradient, boxShadow: '0 0 0 1px rgba(255,255,255,0.15) inset, 0 4px 16px rgba(0,0,0,0.4)' }}>
            <Award size={32} />
          </div>
          <div className="flex flex-col items-end gap-2">
            <span className="text-sm font-semibold tracking-wider text-gray-400">NEUPC</span>
            {cert?.verified && <Pill tone="emerald" icon={Check}>Verified</Pill>}
          </div>
        </div>
        <div className="px-6 py-6 sm:px-8 sm:py-8">
          <h2 className="mb-2 text-xl font-bold leading-snug text-white">{cert?.title}</h2>
          {cert?.description && <p className="mb-6 text-[13px] leading-relaxed text-gray-400">{cert.description}</p>}
          <div className="mb-6 grid grid-cols-2 gap-3 sm:gap-4">
            <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
              <p className="mb-1 text-[11px] font-semibold uppercase tracking-wider text-gray-500">Awarded</p>
              <p className="text-sm font-medium text-gray-200">{fmtDate(cert?.issue_date, { month: 'long', day: 'numeric', year: 'numeric' })}</p>
            </div>
            <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
              <p className="mb-1 text-[11px] font-semibold uppercase tracking-wider text-gray-500">Certificate ID</p>
              <p className="break-all font-mono text-sm font-medium text-gray-200">{cert?.certificate_number}</p>
            </div>
            {cert?.verified && (
              <div className="col-span-2 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4">
                <div className="flex items-center gap-2.5">
                  <Shield size={18} className="text-emerald-400" />
                  <p className="text-[13px] font-medium text-emerald-300">This certificate is verified and authentic</p>
                </div>
              </div>
            )}
            {linkedName && (
          ...
          <div className="flex gap-3">
            {cert?.certificate_url ? (
              <a href={cert.certificate_url} target="_blank" rel="noreferrer" className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-blue-500/30 bg-blue-500/10 px-4 py-3 text-[13px] font-semibold text-blue-300 transition hover:bg-blue-500/20"><Download size={16} /> Download PDF</a>
            ) : (
              <div className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-3 text-[13px] font-medium text-gray-500">No download available</div>
            )}
            <button onClick={onClose} className="rounded-xl border border-white/[0.1] bg-white/[0.04] px-6 py-3 text-[13px] font-semibold text-gray-300 transition hover:bg-white/[0.08] hover:text-white">Close</button>
          </div>
        </div>
      </motion.div>
    </div>
...
function CertificatesTab({ certificates }) {
  ...
  const filtered = useMemo(() => certificates.filter((c) => typeFilter === 'all' || c.certificate_type === typeFilter), [certificates, typeFilter]);

  if (!certificates.length) return <EmptyState icon={Award} title="No certificates yet" description="Attend events and contests to earn certificates." />;

  const tabs = [
    { value: 'all', label: 'All', count: certificates.length },
    ...Object.keys(CERT_TYPE).filter((t) => certificates.some((c) => c.certificate_type === t)).map((t) => ({
      value: t, label: CERT_TYPE[t].label, count: certificates.filter((c) => c.certificate_type === t).length,
    })),
  ];

  return (
    <div className="space-y-4">
      <AnimatePresence>{selected && <CertModal cert={selected} onClose={() => setSelected(null)} />}</AnimatePresence>
      {tabs.length > 2 && <TabBar tabs={tabs} value={typeFilter} onChange={setTypeFilter} />}
          ...
          return (
            <motion.div key={cert.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: i * 0.05 }} onClick={() => setSelected(cert)} className="group cursor-pointer overflow-hidden rounded-xl border border-white/[0.06] bg-white/[0.02] transition-all hover:border-white/[0.12] hover:bg-white/[0.04]">
              <div className="flex items-center justify-between gap-4 border-b border-white/[0.06] px-4 py-4" style={{ background: `radial-gradient(circle at 80% 30%, ${meta.bannerGlow}, transparent 60%), rgba(255,255,255,0.01)` }}>
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full text-white shadow-lg transition-transform group-hover:scale-105" style={{ background: meta.sealGradient, boxShadow: '0 0 0 1px rgba(255,255,255,0.10) inset, 0 4px 12px rgba(0,0,0,0.30)' }}><Award size={22} /></div>
                <div className="flex flex-col items-end gap-1.5"><span className="text-[10px] font-semibold tracking-wider text-gray-500">NEUPC</span>{cert.verified && <Pill tone="emerald" icon={Check}>Verified</Pill>}</div>
              </div>
              <div className="flex flex-col gap-2 px-4 py-3.5">
                <h3 className="line-clamp-2 text-[14px] font-semibold leading-snug text-gray-200">{cert.title}</h3>
                {linkedTo && <p className="truncate text-xs text-gray-400">{linkedTo}</p>}
                <div className="grid grid-cols-2 gap-2 border-t border-white/[0.06] pt-2.5 mt-1">
                  <div><p className="mb-0.5 text-[9px] font-medium uppercase tracking-wider text-gray-500">Awarded</p><p className="font-mono text-[11px] text-gray-300">{fmtDate(cert.issue_date)}</p></div>
                  <div><p className="mb-0.5 text-[9px] font-medium uppercase tracking-wider text-gray-500">ID</p><p className="truncate font-mono text-[11px] text-gray-300">{cert.certificate_number}</p></div>
                </div>
              </div>
            </motion.div>
          );
...
function DiscussionsTab({ myThreads }) {
  if (!myThreads.length)
    return <EmptyState icon={MessageSquare} title="No discussion threads" description="Start a thread in Discussions to see it here." />;

  return (
    <div className="flex flex-col">
      {myThreads.map((thread, i) => (
        <motion.div key={thread.id} initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="flex gap-4 border-b border-white/[0.06] py-3 last:border-b-0 items-center">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/20"><MessageSquare size={16} /></div>
          <div className="flex min-w-0 flex-1 flex-col gap-1">
            <div className="flex flex-wrap items-center gap-2">
              {thread.is_solved && <Pill tone="emerald" icon={CheckCircle2}>Solved</Pill>}
              {thread.tags?.slice(0, 2).map((tag) => <Pill key={tag} tone="gray">#{tag}</Pill>)}
            </div>
            <div className="text-[14px] font-medium leading-snug text-gray-200">{thread.title}</div>
            <div className="flex flex-wrap gap-3 text-xs text-gray-500">
              <span className="flex items-center gap-1"><Eye size={12} /> {thread.views ?? 0} views</span>
              <span className="flex items-center gap-1"><Clock size={12} /> {timeAgo(thread.created_at)}</span>
            </div>
          </div>
          <a href={`/account/member/discussions/${thread.id}`} className="text-gray-500 hover:text-gray-300"><ChevronRight size={18} /></a>
        </motion.div>
      ))}
    </div>
  );
...
const STATS_CONFIG = [
  { id: 'events', title: 'Events', subtext: 'Attended', icon: CalendarDays, tone: 'emerald' },
  { id: 'achievements', title: 'Achievements', subtext: 'Earned', icon: Trophy, tone: 'amber' },
  { id: 'certificates', title: 'Certificates', subtext: 'Verified', icon: Award, tone: 'blue' },
  { id: 'discussions', title: 'Threads', subtext: 'Authored', icon: MessageSquare, tone: 'cyan' },
];
...
export default function MemberParticipationClient({
  ...
  const stats = useMemo(() => {
    ...
    return { events: attended, achievements: earnedAch, certificates: certificates.length, discussions: myThreads.length };
  ...
  const tabCounts = {
    ...
    events: registrations.length, achievements: memberAchievements.length,
    certificates: certificates.length, discussions: myThreads.length,
  };

  const isEmpty = !registrations.length && !memberAchievements.length && !certificates.length && !myThreads.length;
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <div className="space-y-0.5">
            {TABS.map((tab) => {
              ...
              return (
                <button key={tab.id} onClick={() => handleTabChange(tab.id)} className={cn('group/nav relative flex min-h-9 w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-[13px] font-medium transition-colors duration-150 outline-none focus-visible:ring-2 focus-visible:ring-white/30', active ? 'bg-violet-500/12 font-semibold text-violet-400 shadow-violet-500/10' : 'text-gray-400 hover:bg-white/[0.04] hover:text-gray-200')}>
                  {active && <motion.div layoutId="activeTabIndicator" className="absolute left-0 top-1/2 h-4 w-[3px] -translate-y-1/2 rounded-r-full bg-gradient-to-b from-violet-500 to-purple-600" />}
                  <div className="flex items-center gap-3"><Icon className="h-[17px] w-[17px] shrink-0 transition-colors" /><span className="truncate text-left">{tab.label}</span></div>
                  {count > 0 && <span className={cn("px-1.5 py-0.5 rounded-full text-[10px] font-semibold", active ? "bg-violet-500/20 text-violet-300" : "bg-white/[0.06] text-gray-500")}>{count}</span>}
                </button>
              );
            })}
          </div>
          ...
          <div className="flex items-center gap-2 px-4 sm:px-6">
            <nav className="scrollbar-none -mb-px flex items-center gap-0.5 overflow-x-auto flex-1 min-w-0">
              {TABS.map((tab) => {
                ...
                const active = activeTab === tab.id;
                return (
                  <button key={tab.id} onClick={() => handleTabChange(tab.id)} className={cn('flex shrink-0 items-center gap-2 border-b-2 px-3 py-3 text-[13px] font-medium transition-colors', active ? 'border-violet-500 text-white' : 'border-transparent text-gray-500 hover:text-gray-300')}>
          ...
          <div className="mx-auto w-full max-w-7xl flex flex-col gap-8">
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-white tracking-tight mb-1.5">My Activity</h1>
                  <p className="text-sm text-gray-400">A complete record of your participation and achievements</p>
                </div>
              </div>
            </div>
