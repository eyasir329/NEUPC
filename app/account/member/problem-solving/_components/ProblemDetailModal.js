...
const TABS = [
  { id: 'Problem',     icon: FileText,    label: 'Problem',     accent: 'text-violet-400',  gradient: 'from-violet-500 to-purple-500',  hint: 'Statement, examples, constraints' },
  { id: 'Submissions', icon: Clock,       label: 'Submissions', accent: 'text-emerald-400', gradient: 'from-emerald-500 to-teal-500',   hint: 'Your attempts and others’' },
  { id: 'AI Analysis', icon: Sparkles,    label: 'AI Analysis', accent: 'text-indigo-400',  gradient: 'from-indigo-500 to-blue-500',    hint: 'Smart breakdown & chat' },
  { id: 'Similar',     icon: Lightbulb,   label: 'Similar',     accent: 'text-amber-400',   gradient: 'from-amber-500 to-orange-500',   hint: 'Related practice' },
  { id: 'Notes',       icon: StickyNote,  label: 'Notes',       accent: 'text-purple-400',  gradient: 'from-purple-500 to-pink-500',    hint: 'Personal & club notes' },
];

const DIFFICULTY_CONFIG = {
  Easy:   { text: 'text-emerald-400', bg: 'bg-emerald-400/10', border: 'border-emerald-400/20', dot: 'bg-emerald-400' },
  Medium: { text: 'text-amber-400',   bg: 'bg-amber-400/10',   border: 'border-amber-400/20',   dot: 'bg-amber-400' },
  Hard:   { text: 'text-rose-400',    bg: 'bg-rose-400/10',    border: 'border-rose-400/20',    dot: 'bg-rose-400' },
};
...
function normalizeProblem(problem) {
  ...
  const difficulty =
    problem.difficulty ||
    (rating > 0 && rating < 1400 ? 'Easy' : rating < 1900 ? 'Medium' : rating >= 1900 ? 'Hard' : 'Easy');
  return {
    platform,
    problemId,
    problemName: problem.problem_name || problem.problemName || problemId || 'Problem',
    difficulty,
    difficultyRating: rating || 800,
    tags: problem.tags || [],
    verdict: isAc ? 'AC' : verdict || 'AC',
    submittedAt: problem.submitted_at || problem.submittedAt || new Date().toISOString(),
  };
}
...
export default function ProblemDetailModal({ problem, onClose }) {
  ...
  const diffStyle = DIFFICULTY_CONFIG[tabData.difficulty] || DIFFICULTY_CONFIG.Easy;
  const platformStyle = PLATFORM_STYLE[tabData.platform.toLowerCase()] || 'border-white/8 bg-white/4 text-zinc-400';
  ...
  return createPortal(
    <motion.div
      ...
      className="fixed inset-0 z-100 flex min-h-0 flex-col overflow-hidden bg-[#09090b] text-zinc-200 font-sans selection:bg-violet-500/30"
      style={{
        backgroundImage:
        ...
        <div className="flex min-w-0 flex-1 items-center gap-3">
          ...
          <div className="flex min-w-0 items-center gap-2">
            <span className="hidden shrink-0 font-mono text-[10px] tracking-widest text-zinc-600 uppercase sm:inline">
              #{String(tabData.problemId).split(/[-/]/).pop().slice(0, 6).padStart(4, '0')}
            </span>
          ...
          {TABS.map((tab, idx) => {
            ...
            return (
              <button
                ...
                className={`group relative flex shrink-0 items-center gap-2 rounded-lg px-3 py-2.5 my-1.5 text-xs font-medium tracking-wide transition-colors whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/40 ${
                  isActive
                    ? 'text-white'
                ...
                {isActive && (
                  <motion.div
                    ...
                    className={`absolute inset-0 rounded-lg border bg-transparent ${
                      tab.id === 'Problem'     ? 'border-violet-500/30'  :
                      tab.id === 'Submissions' ? 'border-emerald-500/30' :
                      tab.id === 'AI Analysis' ? 'border-indigo-500/30'  :
                      tab.id === 'Similar'     ? 'border-amber-500/30'   :
        ...
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4 border-t border-white/5 px-4 py-3 md:px-6">
          <div className="hidden items-center gap-3 text-[10px] font-medium text-zinc-600 sm:flex">
            <span className="flex items-center gap-1.5">
              <kbd className="rounded border border-white/8 bg-white/4 px-1.5 py-0.5 font-mono text-[9px] text-zinc-400">Esc</kbd>
              <span>close</span>
            </span>
            <span className="flex items-center gap-1.5">
              <kbd className="rounded border border-white/8 bg-white/4 px-1.5 py-0.5 font-mono text-[9px] text-zinc-400">←</kbd>
              <kbd className="rounded border border-white/8 bg-white/4 px-1.5 py-0.5 font-mono text-[9px] text-zinc-400">→</kbd>
              <span>switch tabs</span>
            </span>
            <span className="flex items-center gap-1.5">
              <kbd className="rounded border border-white/8 bg-white/4 px-1.5 py-0.5 font-mono text-[9px] text-zinc-400">1–5</kbd>
              <span>jump to tab</span>
            </span>
          </div>
          <div className="ml-auto text-[10px] font-mono tracking-widest text-zinc-700 uppercase">
            {activeTabMeta.label}
          </div>
        </div>
      </main>
