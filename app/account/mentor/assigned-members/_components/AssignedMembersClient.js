/**
 * @file Assigned members client component
 * @module AssignedMembersClient
 */
...
export default function AssignedMembersClient({ bootcamps }) {
  ...
  const handleRemove = async (id, name) => {
    if (
      !confirm(
        `Remove ${name} from this bootcamp? This deletes their progress.`
      )
    )
      return;
    try {
  ...
  const expandRow = async (student) => {
    ...
    if (!lessonCache[cacheKey]) {
      ...
      try {
        const stats = await getMentorStudentDetailedStats(
          student.bootcampId,
          student.user_id
        );
        setLessonCache((p) => ({
          ...p,
          [cacheKey]: { loading: false, ...stats },
        }));
      } catch {
        setLessonCache((p) => ({
          ...p,
          [cacheKey]: { loading: false, curriculum: null },
        }));
      }
    }
  };
  ...
  const selectedBootcamp = useMemo(
    () => bootcamps.find((bc) => bc.id === bootcampFilter),
    [bootcamps, bootcampFilter]
  );
  const isSelectedArchived = selectedBootcamp
    ? selectedBootcamp.status !== 'published'
    : false;
  ...
  const pending = useMemo(
    () =>
      allEnrollments.filter(
        (e) =>
          e.status === 'pending' &&
          (bootcampFilter === 'all' || e.bootcampId === bootcampFilter)
      ),
  ...
  const students = useMemo(() => {
    return allEnrollments.filter((e) => {
      ...
      if (bootcampFilter !== 'all' && e.bootcampId !== bootcampFilter)
        return false;
  ...
  return (
    <PageShell>
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-zinc-900/50 p-3 shadow-lg backdrop-blur-xl">
        ...
        <div>
          <span className="mb-2 block px-1 text-[10px] font-bold tracking-wider text-violet-400 uppercase">
            Active Cohorts
          </span>
          <div className="flex flex-wrap gap-1.5">
            <FilterTab
              active={bootcampFilter === 'all'}
            ...
            {activeBootcamps.map((bc) => {
              ...
              const count = info.enrollments.filter(
                (e) => e.status !== 'pending'
              ).length;
              return (
                <FilterTab
          ...
          <div className="relative z-10 grid grid-cols-1 gap-4 md:grid-cols-2">
            {pending.map((req) => {
              ...
              return (
                <div
                  key={req.id}
                  className="relative flex flex-col gap-3.5 rounded-2xl border border-white/10 bg-zinc-950/60 p-4 transition-all hover:border-white/20 hover:bg-zinc-950/80"
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="flex shrink-0 items-center gap-1 font-mono text-[9px] text-gray-500">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="inline-flex flex-1 cursor-pointer items-center justify-center gap-1.5 rounded-xl border border-white/[0.06] bg-white/2 py-2 text-[10px] font-bold tracking-wider uppercase transition duration-300 hover:border-rose-500/20 hover:bg-rose-500/10 hover:text-rose-300"
                    >
                      <X className="h-3 w-3" />
                      Decline
                    </button>
                    <button
                      ...
                      className="inline-flex flex-1 cursor-pointer items-center justify-center gap-1.5 rounded-xl border border-emerald-500/20 bg-emerald-500/10 py-2 text-[10px] font-bold tracking-wider text-emerald-300 uppercase transition duration-300 hover:bg-emerald-500/20"
                    >
        ...
        <div className="flex w-full items-center gap-3 md:w-auto">
          <div className="relative w-full md:w-44">
            <select
              ...
              className="w-full cursor-pointer appearance-none rounded-xl border border-white/10 bg-black/20 px-3.5 py-2.5 pr-8 text-sm text-zinc-200 transition-all outline-none hover:bg-black/30 focus:border-violet-500/50 focus:ring-2 focus:ring-violet-500/20"
            >
      ...
      <AnimatePresence>
        {drawerStudent && (
          <StudentDrawer
            ...
            detailedStats={
              lessonCache[
                `${drawerStudent.bootcampId}:${drawerStudent.user_id}`
              ]
            }
          />
        )}
...
function FilterTab({ active, onClick, count, children }) {
  return (
    ...
    >
      ...
      {typeof count === 'number' && (
        <span
          className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold tabular-nums ${active ? 'bg-violet-500/20 text-violet-200' : 'bg-white/5 text-zinc-500'}`}
        >
          {count}
        </span>
      )}
...
function StudentRow({
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-2xl border shadow-lg shadow-black/30 backdrop-blur-xl transition-all duration-300 ${
        expanded
    ...
    >
      <div
        className={`pointer-events-none absolute -top-16 -right-16 h-32 w-32 rounded-full blur-[80px] transition-opacity ${expanded ? 'bg-violet-500/15 opacity-100' : 'bg-violet-500/5 opacity-0 group-hover:opacity-100'}`}
      />
      {expanded && (
        <div className="absolute top-0 bottom-0 left-0 w-[3px] bg-linear-to-b from-violet-400 to-fuchsia-500" />
      )}
      ...
      >
        ...
        <div className="hidden max-w-50 min-w-0 shrink-0 text-left md:block">
          <p className="truncate text-xs font-semibold text-violet-300">
            {student.bootcampTitle.split(':')[0]}
          </p>
        ...
        <div className="w-full shrink-0 space-y-2 md:w-52">
          ...
          <div className="space-y-1">
            ...
            <GradientBar
              value={pct}
              tone={student.status === 'completed' ? 'emerald' : 'violet'}
              height="h-1.5"
            />
          </div>
        </div>
        ...
        <div className="flex shrink-0 items-center justify-between gap-3 md:justify-end">
          <Pill tone={tone}>
            <span
              className={`mr-1 h-1.5 w-1.5 rounded-full bg-current ${student.status === 'active' ? 'animate-pulse shadow-[0_0_6px_currentColor]' : ''}`}
            />
            {student.status}
          </Pill>
          <ChevronRight
            className={`h-4 w-4 text-zinc-500 transition-transform duration-300 ${expanded ? 'rotate-90 text-violet-400' : ''}`}
          />
        </div>
      </div>
            ...
            {!detailedStats?.loading && detailedStats?.scoreBreakdown && (
              <div className="grid grid-cols-4 gap-2">
                {[
                  {
                    label: 'Lessons',
                    value: detailedStats.scoreBreakdown.lesson,
                    color: 'text-violet-400',
                    bg: 'bg-violet-500/10 border-violet-500/20',
                  },
                  {
                    label: 'Tasks',
                    value: detailedStats.scoreBreakdown.task,
                    color: 'text-emerald-400',
                    bg: 'bg-emerald-500/10 border-emerald-500/20',
                  },
                  {
                    label: 'Exams',
                    value: detailedStats.scoreBreakdown.exam,
                    color: 'text-fuchsia-400',
                    bg: 'bg-fuchsia-500/10 border-fuchsia-500/20',
                  },
                  {
                    label: 'Sessions',
                    value: detailedStats.scoreBreakdown.session,
                    color: 'text-sky-400',
                  ...
                  <div
                    key={s.label}
                    className={`rounded-xl border ${s.bg} px-2 py-2.5 text-center`}
                  >
                    <p className={`text-sm font-black tabular-nums ${s.color}`}>
                      {s.value}
                    </p>
                    <p className="mt-0.5 text-[9px] font-bold tracking-wider text-zinc-500 uppercase">
                      {s.label}
                    </p>
                  </div>
                ))}
                  ...
                  <div className="grid max-h-52 grid-cols-1 gap-1.5 overflow-y-auto pr-1 sm:grid-cols-2">
                    {flatLessons.map((l) => {
                      ...
                      return (
                        <div
                          key={l.id}
                          className="flex items-center gap-2.5 rounded-lg border border-white/5 bg-white/2 px-3 py-2 text-xs transition hover:border-white/10"
                        >
                          {done ? (
                          ...
                          <span
                            className={`flex-1 truncate ${done ? 'text-zinc-400' : 'text-zinc-200'}`}
                          >
                            {l.title}
                          </span>
                          {exam?.status === 'graded' && (
                          ...
                          {done && l.progress?.watch_time > 0 && (
                            <span className="shrink-0 font-mono text-[9px] text-zinc-600">
                              {formatWatchSecs(l.progress.watch_time)}
                            </span>
                          )}
                        </div>
                      );
