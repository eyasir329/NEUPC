      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="relative z-10 flex items-center justify-between border-b border-white/5 bg-white/2 px-5 py-4">
            ...
            <button onClick={onClose} className="cursor-pointer rounded-lg p-2 text-zinc-500 transition hover:bg-white/5 hover:text-white">
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <div className="relative z-10 space-y-4 p-5">
            {/* Bootcamp select */}
            <div>
              <label className="mb-1.5 block text-[10px] font-bold tracking-widest text-zinc-500 uppercase">Bootcamp</label>
              <div className="relative">
                <select value={bootcampId} onChange={e => { setBootcampId(e.target.value); setSelected([]); }}
                  className="w-full cursor-pointer appearance-none rounded-xl border border-white/10 bg-black/20 py-2.5 pr-8 pl-3.5 text-sm text-zinc-200 outline-none transition-all focus:border-violet-500/50 focus:ring-2 focus:ring-violet-500/20">
            ...
            {/* Search */}
            <div>
              <label className="mb-1.5 block text-[10px] font-bold tracking-widest text-zinc-500 uppercase">Search candidates</label>
              <div className="group relative">
                ...
                {searching && <Loader2 className="absolute top-1/2 right-3.5 h-4 w-4 -translate-y-1/2 animate-spin text-violet-400" />}
              </div>

              {results.length > 0 && (
                <div className="mt-2 max-h-40 divide-y divide-white/5 overflow-y-auto rounded-xl border border-white/10 bg-zinc-950/80">
                  {results.map(u => (
                    <button key={u.id} onClick={() => pick(u)} className="flex w-full cursor-pointer items-center gap-2.5 px-3 py-2 text-left transition-colors hover:bg-white/5">
                      {u.avatar_url
                        // eslint-disable-next-line @next/next/no-img-element
                        ? <img src={u.avatar_url} className="h-7 w-7 rounded-full object-cover ring-1 ring-white/10" alt="" />
                        : <div className="flex h-7 w-7 items-center justify-center rounded-full border border-violet-500/20 bg-violet-500/15 text-[10px] font-bold text-violet-300">{u.full_name?.[0] || '?'}</div>}
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-semibold text-white">{u.full_name || 'Unknown'}</p>
              ...
              {query.length >= 2 && !searching && results.length === 0 && (
                <p className="mt-2 text-center text-xs text-zinc-600">No users found for &quot;{query}&quot;</p>
              )}
            </div>
            ...
            {selected.length > 0 && (
              <div>
                <p className="mb-1.5 text-[10px] font-bold tracking-widest text-zinc-500 uppercase">Selected ({selected.length})</p>
                <div className="flex flex-wrap gap-1.5">
                  {selected.map(u => (
                    <span key={u.id} className="inline-flex items-center gap-1.5 rounded-full border border-violet-500/20 bg-violet-500/10 py-1 pr-1.5 pl-2.5 text-xs text-violet-200">
                      {u.full_name || u.email}
                      <button onClick={() => remove(u.id)} className="flex h-4 w-4 cursor-pointer items-center justify-center rounded-full text-violet-400 transition-colors hover:bg-violet-500/30 hover:text-white">
            ...
            <div className="flex items-center justify-end gap-2 border-t border-white/5 pt-3">
              <button onClick={onClose} className="cursor-pointer rounded-lg px-4 py-2 text-sm font-medium text-zinc-400 transition-all hover:bg-white/5 hover:text-white">Cancel</button>
