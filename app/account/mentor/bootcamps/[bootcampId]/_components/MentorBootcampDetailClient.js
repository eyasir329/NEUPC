/**
 * @file Mentor bootcamp detail client component
 * @module MentorBootcampDetailClient
 */
      ...
      <PageHeader
        ...
        meta={
          <>
            <Pill tone={tone}>
              <span
                className={`h-1.5 w-1.5 rounded-full ${sc.dot} ${bootcamp.status === 'published' ? 'animate-pulse' : ''} mr-1`}
              />
              {sc.label}
            </Pill>
        ...
        <div className="mb-6 flex items-center gap-3 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4 text-sm text-amber-200">
          ...
          <div className="flex-1">
            <p className="text-xs font-semibold tracking-wider text-amber-400 uppercase">
              Archived Cohort (Read-Only)
            </p>
            <p className="mt-0.5 text-xs text-amber-200/80">
              This bootcamp has been archived. You can view the curriculum and
              lesson contents, but all syllabus edits, module additions, and
              lesson updates are disabled.
            </p>
          </div>
        </div>
      )}
