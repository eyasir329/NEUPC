...
export default function MentorSettingsClient({ user }) {
  ...
  return (
    <PageShell>
      <PageHeader icon={Settings} title="Settings" subtitle="Configure your mentor account preferences" accent="blue" />
      ...
      <GlassCard padding="p-6">
        ...
        <div className="space-y-3">
          {[
            ...
            { label: 'Account Status', value: user?.account_status, badge: user?.is_online ? { text: 'Active', tone: 'emerald' } : { text: 'Inactive', tone: 'rose' } },
          ].map(({ label, value, note, badge }) => (
            <div key={label} className="flex items-center justify-between rounded-xl border border-white/6 bg-white/2 px-4 py-3">
              <div>
              ...
              {badge && (
                <span className={`rounded-full px-2 py-0.5 text-xs ${badge.tone === 'emerald' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>{badge.text}</span>
              )}
            </div>
          ))}
        </div>
      ...
      <GlassCard padding="p-6">
        ...
        <form onSubmit={handleSaveNotifications} className="space-y-3">
          {[
            { key: 'session_reminders', label: 'Session Reminders', desc: 'Get reminded before scheduled sessions' },
            { key: 'task_submissions', label: 'Task Submissions', desc: 'Notify when a mentee submits a task' },
            { key: 'mentee_updates', label: 'Mentee Updates', desc: 'Activity updates from your mentees' },
            { key: 'weekly_digest', label: 'Weekly Digest', desc: 'Weekly summary of mentorship activity' },
          ].map(({ key, label, desc }) => (
            <div key={key} className="flex items-center justify-between rounded-xl border border-white/6 bg-white/2 px-4 py-3">
              ...
              <Toggle checked={notifSettings[key]} onChange={(val) => setNotifSettings({ ...notifSettings, [key]: val })} />
            </div>
          ))}
          <div className="pt-2">
            <button type="submit" disabled={savingNotif} className="flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50">
