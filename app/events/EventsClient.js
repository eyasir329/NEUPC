...
function getCover(event) {
  return driveImageUrl(
    event.cover_image || event.banner_image || event.image || ''
  );
}
    ...
    >
      ...
      >
        ...
        <div className="flex flex-1 flex-col gap-2 px-0.5">
          <h3 className="font-heading group-hover:text-neon-lime text-base leading-snug font-black text-white transition-colors duration-200 sm:text-lg">
            {event.title}
          </h3>
          {event.description && (
...
function FeaturedBanner({ event }) {
  ...
  return (
    <article className="group relative overflow-hidden rounded-2xl border border-white/8 bg-[#08090f] shadow-[0_0_0_1px_rgba(182,243,107,0.06),0_32px_64px_-16px_rgba(0,0,0,0.7)]">
      ...
      <div className="via-neon-lime/60 absolute inset-x-0 top-0 z-10 h-px bg-linear-to-r from-transparent to-transparent" />
      ...
      <div className="relative aspect-[3/2] w-full overflow-hidden sm:aspect-[16/8]">
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-24 bg-linear-to-t from-[#08090f] to-transparent" />

        {/* Top-left badges */}
        <div className="absolute top-4 left-4 flex items-center gap-2">
          <span className="border-neon-lime/25 text-neon-lime inline-flex items-center gap-1.5 rounded-full border bg-black/60 px-3 py-1 font-mono text-[9px] font-bold tracking-[0.2em] uppercase backdrop-blur-xl sm:text-[10px]">
            <span className="bg-neon-lime h-1.5 w-1.5 rounded-full shadow-[0_0_6px_2px_rgba(182,243,107,0.7)]" />
            Featured
          </span>
          <span
            className={cn(
      ...
      <div className="relative -mt-6 px-6 pb-7 sm:-mt-8 sm:px-8 sm:pb-9 lg:px-10 lg:pb-10">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-x-4 gap-y-2">
          {event.start_date && (
            <span className="text-neon-lime font-mono text-[10px] tracking-[0.25em] uppercase sm:text-[11px]">
        ...
        {/* Title */}
        <h3 className="kinetic-headline font-heading text-[1.6rem] leading-[1.05] font-black text-white uppercase sm:text-4xl lg:text-5xl">
          {event.title}
        </h3>

        {/* Neon-lime underline accent */}
        <div className="bg-neon-lime/50 mt-4 h-px w-12 sm:mt-5" />

        {/* Description */}
        {event.description && (
          <p className="mt-4 max-w-2xl text-sm leading-[1.75] text-zinc-400 sm:mt-5 sm:text-[15px]">
            {event.description.length > 220
              ? `${event.description.slice(0, 220).trim()}…`
              : event.description}
          </p>
        )}
        ...
        <div className="mt-6 flex flex-wrap items-center gap-4 sm:mt-7">
          ...
          {event.location && (
            <span className="flex items-center gap-2 font-mono text-[10px] tracking-wider text-zinc-500 sm:text-[11px]">
              <svg
                className="text-neon-lime/60 h-3.5 w-3.5 shrink-0"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2}
...
function ArchiveRow({ event }) {
  return (
    <Link
      href={getHref(event)}
      className="group flex items-center justify-between gap-4 border-b border-white/5 px-2 py-5 transition-colors hover:bg-white/[0.02] sm:px-4 sm:py-6"
    >
      <div className="min-w-0 flex-1">
        <h4 className="font-heading group-hover:text-neon-lime truncate text-base font-black text-white transition-colors sm:text-lg">
          {event.title}
        </h4>
      ...
      <span className="group-hover:text-neon-lime shrink-0 font-mono text-[9px] tracking-widest text-zinc-600 uppercase transition-all group-hover:translate-x-1 sm:text-[10px]">
        View →
      </span>
    </Link>
  );
...
export default function EventsClient({
  ...
  const counts = useMemo(
    () => ({
      all: events.length,
      active: events.filter((e) => ['upcoming', 'ongoing'].includes(e.status))
        .length,
            ...
            {archiveEvents.length > 5 && (
              <div className="mt-8 text-center sm:mt-10">
                <button
                  ...
                  className="font-heading hover:border-neon-lime/35 hover:text-neon-lime rounded-full border border-white/12 px-8 py-3 text-[10px] font-bold tracking-widest text-zinc-400 uppercase transition-all sm:px-10 sm:py-3.5 sm:text-[11px]"
                >
                  {showAllArchive
                    ? 'Show Less'
