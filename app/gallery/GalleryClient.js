  ...
  return (
    ...
    >
      <button
        ...
        className="group holographic-card focus-visible:ring-neon-lime w-full overflow-hidden rounded-xl text-left focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-[#05060b] focus-visible:outline-none"
      >
        {/* Image */}
            ...
            <div className="flex h-12 w-12 items-center justify-center rounded-full border border-white/30 bg-black/50 backdrop-blur-md">
              ...
              >
                <path
                  ...
                  d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607zM10 7v6m3-3H7"
                />
              </svg>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-1.5 p-4 sm:p-5">
          <h3 className="font-heading group-hover:text-neon-lime line-clamp-2 text-base leading-snug font-black text-white transition-colors sm:text-lg">
            {item.title || 'Untitled'}
          </h3>
          {item.description && (
...
function LightboxNavBtn({ direction, onClick }) {
  ...
  return (
    <button
      ...
      className={cn(
        'hover:border-neon-lime/50 hover:bg-neon-lime/15 hover:text-neon-lime absolute top-1/2 z-50 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-white/20 bg-white/10 text-white backdrop-blur-xl transition-all duration-300 hover:scale-110 sm:h-12 sm:w-12',
...
export default function GalleryClient({
  ...
  const years = useMemo(
    () =>
      [...new Set(galleryItems.map((i) => i.year).filter(Boolean))].sort(
        (a, b) => b - a
      ),
    [galleryItems]
  );
  ...
  const filteredItems = useMemo(() => {
    ...
    if (activeYear !== 'all')
      items = items.filter((i) => i.year === activeYear);
    if (search.trim()) {
  ...
  useEffect(() => {
    setPage(1);
  }, [activeFilter, activeYear, search, sortBy]);
  ...
  const hasFilters =
    activeFilter !== 'all' || activeYear !== 'all' || search.trim() !== '';
  function clearAll() {
    setSearch('');
    setFilter('all');
    setYear('all');
          ...
          >
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              ...
              <div ref={sortRef} className="relative shrink-0">
                ...
                {showSort && (
                  <div className="absolute top-full right-0 z-30 mt-1.5 w-44 overflow-hidden rounded-xl border border-white/10 bg-[#0e1018] shadow-xl shadow-black/50">
              ...
              <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#0c0e16] shadow-2xl">
                ...
                <div className="border-t border-white/8 p-5 sm:p-7">
                  <div className="mb-3 flex flex-wrap items-center gap-2">
                    <span className="bg-neon-lime/90 rounded-full px-3 py-0.5 font-mono text-[9px] font-bold tracking-widest text-black uppercase">
                      {CATEGORY_META[lightbox.category]?.label ||
                        lightbox.category}
                    </span>
                    {lightbox.year && (
                  ...
                  <h2 className="font-heading mb-2 text-xl leading-tight font-black text-white sm:text-2xl lg:text-3xl">
                    {lightbox.title || 'Untitled'}
                  </h2>
                  {lightbox.description && (
                    <p className="mb-3 line-clamp-3 text-sm leading-relaxed text-zinc-400 sm:text-base">
                      {lightbox.description}
                    </p>
                  )}
                  {lightbox.date && (
