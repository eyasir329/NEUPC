/**
 * @file Roadmaps client component
...
function RoadmapCard({ roadmap }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Cover */}
        <div className="relative mb-4 aspect-video w-full overflow-hidden rounded-xl border border-white/8 bg-white/3 shadow-sm transition-shadow duration-300 group-hover:shadow-lg group-hover:shadow-black/40">
          {image ? (
            <SafeImg
              src={image}
              alt={roadmap.title}
...
export default function RoadmapsClient({
  ...
  const beginnerCount = useMemo(
    () => roadmaps.filter((r) => r.difficulty === 'beginner').length,
    [roadmaps]
  );
  ...
  }, [roadmaps, difficulty, search, sortBy]);
          ...
          >
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              <SortSelect
                ...
                ariaLabel="Sort roadmaps"
              />
            </div>

            {/* Difficulty tabs + clear */}
                ...
                <div className="scrollbar-none -mx-1 flex gap-1.5 overflow-x-auto px-1 py-0.5">
                  {difficultyTabs.map((tab) => (
                    <button
                      ...
                      aria-pressed={difficulty === tab.id}
                      className={cn(
