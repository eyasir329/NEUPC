/**
 * NEUPC CS Academy Extractor (Standalone)
 * Self-contained script without ES module imports for Firefox compatibility
 * Extracts submission data from CS Academy
 */
...
(function () {
  ...
  // ============================================================
  // CS ACADEMY EXTRACTOR
  // ============================================================

  class CSAcademyExtractor {
    constructor() {
    ...
    detectPageType() {
      ...
      if (path.includes('/contest/') && path.includes('/task/')) {
        return 'problem';
      }
    ...
    async extractSubmission() {
      try {
        ...
        await waitForElement(
          '.submission-details, [class*="submission"]',
          2000
        ).catch(() => null);

        // Extract problem info
        const problemLink = safeQuery('a[href*="/task/"]');
        let problemId = null;
        let problemUrl = null;
        ...
        if (problemLink) {
          ...
          const match = problemUrl.match(/\/task\/([^/]+)/);
        ...
        // Extract contest ID
        const contestId =
          window.location.pathname.match(/\/contest\/([^/]+)/)?.[1];
        log('Contest ID:', contestId);
  ...
  function initExtractor() {
    ...
    const extractor = new CSAcademyExtractor();
    extractor.init();
  }
