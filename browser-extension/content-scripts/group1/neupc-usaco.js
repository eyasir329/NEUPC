/**
 * NEUPC USACO Extractor (Standalone)
 * Self-contained script without ES module imports for Firefox compatibility
 * Note: USACO submissions are private - extension support limited
 */
...
(function () {
  ...
  class USACOExtractor {
    ...
    async getUserHandle() {
      // USACO doesn't have public profiles
      return null;
    }

    async extractSubmission() {
      log('USACO submissions are private - extension support limited');
      log('You can manually copy your submission details');
      return null;
    }
    ...
    async init() {
      ...
      if (pageType === 'problem') {
        log('USACO problem page detected');
        log(
          'Note: USACO submissions are private and cannot be automatically extracted'
        );
      } else {
  ...
  function initExtractor() {
    ...
    const extractor = new USACOExtractor();
    extractor.init();
  }
