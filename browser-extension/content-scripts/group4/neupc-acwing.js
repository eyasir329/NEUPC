/**
 * NEUPC AcWing Extractor (Standalone)
 * Self-contained script without ES module imports for Firefox compatibility
 * Extracts submission data from AcWing (acwing.com) - Chinese CP platform
 */

(function () {
  'use strict';

  const PLATFORM = 'acwing';
  ...
  class AcWingExtractor {
    ...
    detectPageType() {
      ...
      if (path.includes('/solution/')) return 'submission';
    ...
    async getUserHandle() {
      const userLink = safeQuery('a[href*="/user/"], .user-info');
      if (userLink) {
    ...
    async extractSubmission() {
      try {
        const solMatch = window.location.pathname.match(/\/solution\/(\d+)/);
        if (!solMatch) {
          log('No solution ID found');
          return null;
        }

        const submissionId = solMatch[1];
        log('Extracting solution:', submissionId);

        await waitForElement('.solution-content, .result, main', 3000).catch(
          () => null
        );
        ...
        // Extract verdict
        let verdict = 'UNKNOWN';
        const statusEl = safeQuery('.result, .status, [class*="status"]');
        if (statusEl) {
        ...
        const statsEls = safeQueryAll('.info-item, .stat, td');
  ...
  // Extract language
    ...
    async extractSourceCode() {
      const selectors = ['.code-content', 'pre code', '.source-code', 'pre'];
