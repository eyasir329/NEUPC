/**
 * NEUPC Yosupo Judge Extractor (Standalone)
 * Self-contained script without ES module imports for Firefox compatibility
 * Extracts submission data from Yosupo Judge (judge.yosupo.jp)
 * Library of competitive programming problems with reference solutions
 */

(function () {
  'use strict';

  const PLATFORM = 'yosupo';
  ...
  class YosupoExtractor {
    ...
    async extractSubmission() {
      try {
        ...
        // Extract verdict - Yosupo shows AC/WA/TLE/etc.
        let verdict = 'UNKNOWN';
        ...
        // Extract time and memory from table or stats
        let executionTime = null,
          memoryUsed = null;
        const rows = safeQueryAll('tr, .stat-row');
  ...
  // Extract language
        ...
        const langEl = safeQuery('.language, [class*="lang"], td:nth-child(4)');
        if (langEl) {
